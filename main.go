...
func init() {
	var err error
	//db, err = sql.Open("postgres", "postgres://surdyhey:Goodman8349**@localhost/productstore")
	db, err = sql.Open("postgres", "user=postgres password=Goodman8349** host=127.0.0.1 port=5432 dbname=productstore sslmode=disable")
	if err != nil {
		log.Fatal(err)
	}
}
